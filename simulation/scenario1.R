...
for(i in 1:(n+nn)){
  p0 = sample(2:10,1); v0=p0*(p0-1)/2
  m0 = sample(4:10,1)
  seed = rbinom(v0,1,0.5)
  g0 = graph_from_adjacency_matrix(func.vtm(seed),mode='undirected')
  g = sample_pa(100,directed=F,start.graph=g0,m=m0)
...
save(mse,coverage,width,hs.edge.prop,lsGPR.prop,
     file=paste0("~/simulation/results/sce1_act",act,"_rep",ind,".Rdata"))
