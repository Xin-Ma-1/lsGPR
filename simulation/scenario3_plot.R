...
for(s in 1:3){
  ...
  for(k in 1:3){
    ...
    for(i in 1:sample){
      load(paste0("sce3_lam",lam,"_act",act.num[k],"_rep",i,".Rdata"))
      MSE[i,] = mse
      COVERAGE[i,] = coverage
      WIDTH[i,] = width
      node.lab = rep(0,p)
  ...
  name = c(rep("Lasso",sample),rep("Ridge",sample),rep("Elastic Net",sample),rep("Horseshoe",sample),
           rep("edgeGPR",sample),rep("pcaGPR",sample),rep("mfGPR",sample),rep("lsGPR",sample),
           rep("sp-lsGPR",sample))
  ...
  plots[[(4*(s-1)+1)]] = ggplot(data=data1, aes(x=active, y=pred.MSE)) + geom_boxplot(aes(fill=method),outlier.shape=NA) +
    scale_fill_manual(values=mypal) + ylim(0,2) + ggtitle(paste0("lambda=",lam)) +
  ...
  name = c(rep("edgeGPR",sample),rep("pcaGPR",sample),rep("mfGPR",sample),rep("lsGPR",sample),
           rep("sp-lsGPR",sample))
  ...
  plots[[(4*(s-1)+2)]] = ggplot(data=data2, aes(x=active, y=coverage)) + geom_boxplot(aes(fill=method),outlier.shape=NA) +
    ylim(0,1) + ggtitle(paste0("lambda=",lam)) +
  ...
  plots[[(4*(s-1)+3)]] = ggplot(data=data3, aes(x=active, y=interval.width)) + geom_boxplot(aes(fill=method),outlier.shape=NA) +
    ylim(0,6) + ggtitle(paste0("lambda=",lam)) +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position="none",
          panel.background = element_blank(), axis.line = element_line(colour = "black"),
          axis.title=element_text(size=18),axis.text=element_text(size=16),
          legend.text=element_text(size=16),legend.title=element_text(size=18),plot.title=element_text(size=20,face='bold')) +
  ...
  plots[[(4*s)]] = ggplot(data=data4, aes(x=active, y=AUC)) + geom_boxplot(aes(fill=method),outlier.shape=NA) +
    ylim(0,1) + ggtitle(paste0("lambda=",lam)) +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position="none",
          panel.background = element_blank(), axis.line = element_line(colour = "black"),
          axis.title=element_text(size=18),axis.text=element_text(size=16),
...
setEPS()
postscript("~/simulation/results/sce3_boxplots.eps",width=16.67,height=15.625)
grid_arrange_shared_legend(plots[[1]],plots[[2]],plots[[3]],plots[[4]],
